...
def calculate_vm_count(build_plan_path):
	...
	for k, v in count_dict.items():
		...
		if k == vm_type and k != '':
			...
			print ("\n", "Please check your vm-type's in build plan. Your preload folder has a file with vm type not specified in the build plan's 'General Tab.'", "\n")
			break 
...
def change_vm(preload_path, build_plan_path, count):
	...
	if int(count) < 10:
			vm_name_val = vnf_name_no_number + ppp + "00" + count
	else:	
			vm_name_val = vnf_name_no_number + ppp + "0" + count
