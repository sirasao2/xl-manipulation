	...
	for i in range(bp.nrows):
		for j in range(bp.ncols):
			...
			if bp.cell_value(i, j) == "vf-module-name":
					col_ref_vfmn = j
	# print("col_ref_vmt", col_ref_vmt)
	# print("col_ref_vmc", col_ref_vmc)
	# print("col_ref_vfmn", col_ref_vfmn)

	# col_names = []
	# for cols in bp.row(3): # 0 indexed, gives all column names from row 5
...
def change_ips(preload_path, build_plan_path):
	...
	# 		col_ref_vmn = col_num
	# 	if col_names[col_num] == "vm-type" and not None:
	# 		col_ref_vmt = col_num
