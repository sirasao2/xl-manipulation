...
def names_tag_sheet(preload_path, build_plan_path):
	...
	# for i in range(5, bp.nrows):
	# 	ppp_list.append(bp.cell_value(i, col_ref_vmt))
	# ppp_list = set(ppp_list)
	# vmtypes = list(ppp_list)
	# print(vmtypes)
	...
	# OLD names_dict = {"vlbagent_eph" : lba_list, "vprb" : prb_list, "qrouter" : qrt_list, }
	names_dict = {
					...
					"Cognos Content Manager" : ccm_list ,
					"Load Balancer" : qlb_list ,
					"gaurdian" : gdn_list ,
					"Schema Database" : dbm_list ,
					"kafka" : akf_list ,
					"distributedlock" : dtl_list ,
					"scheduledservices" : ssr_list ,
					"vertica" : vdb_list ,
					"Log Collector" : log_list ,
					"processing" : imm_list ,
					"rpmrepository" : srp_list ,
					"configrepo" : crp_list ,
					"settingsandhealthdb" : shd_list ,
					"qloader" : ldr_list ,
					"Gateway" : cgw_list ,
					"daemon" : dmn_list ,
					"apigw" : agw_list 
